...
class MealplanManager extends AbstractManagerAdmin
{
    public const TABLE = 'mealplan';

    /**
     * Insert new item in database
     */
    public function insert(array $mealplan): int
    {
        $query = "INSERT INTO " . self::TABLE .
            " (`type`, `price`)
        VALUES (:type, :price)";
        $statement = $this->pdo->prepare($query);
        $statement->bindValue(':type', $mealplan['type'], \PDO::PARAM_STR);
        $statement->bindValue(':price', $mealplan['price'], \PDO::PARAM_STR);
        $statement->execute();
     ...
     */
    public function update(array $mealplan): bool
    {
        $query = "UPDATE " . self::TABLE . " SET `type` = :type, `price` = :price  WHERE id=:id";
        $statement = $this->pdo->prepare($query);
        $statement->bindValue('id', $mealplan['id'], \PDO::PARAM_INT);
        $statement->bindValue('type', $mealplan['type'], \PDO::PARAM_STR);
        $statement->bindValue('price', $mealplan['price'], \PDO::PARAM_STR);
